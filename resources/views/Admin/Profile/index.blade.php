<x-layout.admin>
    ...
    <div class="max-w-3xl mx-auto bg-white shadow-lg rounded-lg p-6 mt-10">
        ...
        <div class="flex items-center space-x-6">
            <form action="{{ route('profile.image.update', parameters: Auth::guard('admin')->user()->id) }}" method="POST" enctype="multipart/form-data" class="relative inline-block">
                @csrf
                @method('PATCH')

                <label for="profile_photo" class="cursor-pointer relative">
                    <img 
                        src="{{ asset('images/' . (Auth::guard('admin')->user()->profile_photo ?? 'profile.jpg')) }}"
                        alt="Profile Photo"
        ...
        <div class="mt-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-700">
                ...
                <div>
                    <p><strong>Joined:</strong> {{ Auth::guard('admin')->user()->created_at->format('d M Y') }}</p>
                    <p><strong>Role:</strong> {{ Auth::guard('admin')->user()->role ?? 'Admin' }}</p>
                </div>
            </div>
        </div>
